...
**The web app's `/trace` view does not show a per-chunk retrieval score
or whether reranking moved a chunk**, both asked for by the original
build spec. `search_chunks` computes cosine distance only to order its
SQL query and never selects the value back out, and the rerank cross
encoder's scores are used the same way in the chat orchestration path:
neither number is persisted anywhere. `/trace` shows the real final
retrieval order and the turn level reranked flag, both true, rather
than inventing a per-chunk number this build never measured. Retrofitting
real persistence would touch the retrieval evaluation harness that
produces every metric on this page, itself already correct and fully
tested, for a web display gap alone, so it is disclosed here rather
than risked this late in the build.
